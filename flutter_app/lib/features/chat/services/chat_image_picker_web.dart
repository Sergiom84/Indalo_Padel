...
const double _jpegQuality = 0.76;
const String _outputMimeType = 'image/jpeg';

Future<String?> pickChatImageAsDataUrlImpl(ImageSource source) {
...
Future<String> _readOptimizedImageAsDataUrl(html.File file) async {
  ...
  return canvas.toDataUrl(_outputMimeType, _jpegQuality);
}
...
(int, int) _scaledDimensions(int width, int height) {
  ...
  if (largestSide <= _maxChatImageDimension) {
    return (width, height);
  }

  final scale = _maxChatImageDimension / largestSide;
  return (
    (width * scale).round(),
    (height * scale).round(),
  );
